...
export const COMMAND_DIAGNOSTIC_MESSAGES = {
  ...
  FIX_PERMISSION: 'run the command from the same user that owns the process, stop the owning application manually, or retry with elevated privileges such as sudo/Admin when appropriate.',
  ISSUE_NO_PROCESS: 'the process exited after discovery but before termination was attempted.',
  FIX_NO_PROCESS: 'rerun port-kill, verify the port is still in use, or restart the owning service before retrying.',
  ...
  FIX_MISSING_COMMAND: 'install the missing system utility or run from a shell where it is available on PATH.',
  ISSUE_FALLBACK: 'the operating system rejected the termination command.',
  FIX_FALLBACK: 'rerun with --verbose, verify the process owner and PID, then try a graceful signal such as --signal SIGTERM or elevated privileges if the process is protected.'
} as const;
